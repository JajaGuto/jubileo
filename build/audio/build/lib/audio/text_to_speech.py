...
class TextToSpeech(Node):
    def __init__(self):
        ...
        self.command_sub = self.create_subscription(
            String, 'generate_audio', self.generate_speech, qos_profile_sensor_data
        )
    ...
    def generate_speech(self, msg):
        ...
        message = Audio()
        sound = AudioSegment.from_mp3("output.mp3").set_frame_rate(44100)
        sound.export("output_jub.wav", format="wav")
        _, data = wav.read('output_jub.wav')
        message.data = np.float32(data * 0.000030514).tolist()
